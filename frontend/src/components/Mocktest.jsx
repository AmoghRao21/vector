...
const MOCK_TESTS = [
  { id: 1, title: "Algebra Basics", subject: "Mathematics", difficulty: "Easy", duration: 60, totalQuestions: 25 },
  { id: 2, title: "Advanced Mechanics", subject: "Physics", difficulty: "Hard", duration: 90, totalQuestions: 40 },
  { id: 3, title: "Organic Chemistry Concepts", subject: "Chemistry", difficulty: "Medium", duration: 75, totalQuestions: 35 },
  { id: 4, title: "Data Structures", subject: "Computer Science", difficulty: "Medium", duration: 80, totalQuestions: 30 },
  { id: 5, title: "Genetics Overview", subject: "Biology", difficulty: "Easy", duration: 45, totalQuestions: 20 },
];
  ...
  return (
    <motion.div
      ...
      className={`group relative bg-gradient-to-br from-[#191919]/90 via-[#26262a]/80 to-[#181818] border border-[#26262a] rounded-2xl shadow-2xl ${
        view === "list" ? "flex-row px-10 py-7 min-h-[100px] flex items-center" : "flex flex-col min-h-[280px] px-7 py-7"
      } justify-between overflow-hidden`}
      style={{ transition: "box-shadow .2s cubic-bezier(.6,.4,0,1)" }}
    >
      <HiOutlineSparkles
        className={`absolute text-[#e99b63]/10 ${
          view === "list" ? "left-2 top-2 text-3xl opacity-30 blur-sm" : "left-8 top-7 text-6xl opacity-40 blur-lg"
        } pointer-events-none`}
        style={{ zIndex: 0 }}
      />
      <div className={`relative z-10 ${view === "list" ? "flex items-center gap-7 flex-1" : ""}`}>
...
export default function MockTestsPage() {
  ...
  const filteredTests = useMemo(() => {
    return MOCK_TESTS.filter((test) => {
      const subjectMatch = subject === "All Subjects" || test.subject === subject;
      const difficultyMatch = difficulty === "All" || test.difficulty === difficulty;
      const searchMatch = test.title.toLowerCase().includes(search.toLowerCase());
  ...
  const particlesOptions = {
    ...
    fpsLimit: 60,
    interactivity: { events: { resize: true }, detectsOn: "canvas" },
    particles: {
      color: { value: "#e99b63" },
      move: { direction: "none", enable: true, random: true, speed: 0.25, outModes: { default: "out" } },
      ...
      shape: { type: "circle" },
      size: { value: { min: 4, max: 12 }, random: true, anim: { enable: true, speed: 2, size_min: 3, sync: false } },
    },
    detectRetina: true,
  };

  return (
    <div className="relative min-h-screen w-full bg-black flex flex-col items-center py-14 px-6">
      <Particles
        id="tsparticles"
        init={particlesInit}
      ...
      />
      <header className="pt-16 pb-6 w-full max-w-7xl flex flex-col items-center z-10 relative">
        <motion.h1
          initial={{ opacity: 0, y: -15 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ delay: 0.2, type: "spring", stiffness: 100 }}
          className="text-6xl font-black tracking-tight text-white text-center"
        >
        ...
        <p className="mt-4 mb-0 text-xl max-w-2xl text-gray-500 font-medium text-center">
          Master your preparation. Filter by subject, difficulty, or search and start instantly.
        </p>
      </header>
      <motion.section
