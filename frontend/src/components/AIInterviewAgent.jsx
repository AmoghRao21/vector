...
const AVATAR_URL = "/interviewer-avatar.svg";

// Particle background similar to other sections
function InterviewParticlesBG() {
...
export default function AIInterviewAgent() {
  const [messages, setMessages] = useState([
    {
      sender: "ai",
      text: "Welcome to your AI Interview. Press the mic and answer. Ready? 'Introduce yourself briefly.'",
    },
  ]);
    ...
    return (
      ...
      >
        ...
        <motion.div
          ...
          ${
            isAI
              ? "bg-[#232226] text-white border-l-4 border-[#e99b63]"
  ...
  return (
    <div className="relative min-h-screen bg-[#131313] overflow-x-hidden">
      {/* Particle Background */}
      <InterviewParticlesBG />

      {/* Blurred elements for depth */}
      <div className="absolute top-1/4 -left-20 w-72 h-72 bg-gradient-to-r from-[#e99b63] to-[#ff6b35] rounded-full blur-3xl opacity-20"></div>
      <div className="absolute bottom-1/4 -right-20 w-72 h-72 bg-gradient-to-l from-[#e99b63] to-[#ff6b35] rounded-full blur-3xl opacity-20"></div>

      {/* Navigation Header with Vector Logo */}
      <motion.header
      ...
      >
        <div className="max-w-7xl mx-auto flex items-center">
          <Link to="/" className="flex items-center space-x-2 group">
            <motion.span
              className="font-['AstronBoy'] text-2xl md:text-3xl font-bold text-white transition-all duration-300 group-hover:text-[#e99b63]"
      ...
      <div className="relative z-10 w-full flex items-center justify-center py-10 px-2">
        <section className="w-full max-w-3xl rounded-3xl shadow-2xl bg-[#191818]/90 backdrop-blur-md border border-[#e99b6329] px-0 sm:px-8 py-10 flex flex-col justify-between">
          <header className="flex items-center gap-4 mb-7 px-6">
            <WaveformBar active={listening} />
            <span className="text-2xl text-[#e99b63] font-extrabold drop-shadow">
              AI Interview Agent
            </span>
            <button
    ...
    <div className="flex items-end h-6 gap-[2px] w-28 mx-auto">
      {[...Array(18)].map((_, i) => (
        <motion.div
          ...
          animate={{
            height: active ? [2, 16 + (i % 5) * 6, 12, 19, 8] : [6, 9, 8, 7, 6],
          }}
          transition={{
            duration: 1 + i * 0.02,
            repeat: Infinity,
